...
namespace {
    ...
    Task* TaskUserConstructor(const CHAR8 *Name, VOID *ELFImage) {
        ...
        // --- CFS SPECIFIC INITIALIZATION ---
        // Karena ini User Task (kayak terminal/shell), kita kasih nilai normal (0)
        newTask->NiceValueOfThisGuy = 0; 
        newTask->Weight = RateThisTaskNice(newTask->NiceValueOfThisGuy);
        
        // Warisin MinVRuntime saat ini biar task baru gak rakus ngambil CPU
        ...
        // Pastikan RBT Node bersih (sebenarnya udah dicover Memset, tapi buat explicit aja)
        newTask->RbtLeft = nullptr;
        newTask->RbtRight = nullptr;
        newTask->RbtParent = nullptr;
        newTask->Color = RBT_RED; // Default node insert biasanya MERAH
